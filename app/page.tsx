...
export default function Home() {
  return <main>
    <header className="topbar"><div className="brand">NEX<span>RIG</span></div><div className="actions"><Search size={21}/><Heart size={21}/><ShoppingCart size={21}/></div></header>
    <section className="hero">
      <div className="eyebrow">NEXT-GEN GAMING HARDWARE</div>
      <h1>Build faster.<br/>Play harder.</h1>
      <p>Curated components, compatibility guidance, and performance-first gaming hardware.</p>
      <div className="heroBtns"><Link href="#shop" className="primary">Shop Components</Link><Link href="#builder" className="ghost">Build Your PC</Link></div>
    ...
    <section id="shop" className="productsWrap">
      ...
      <div className="productGrid">{products.map((p,i)=><Link href={`/product/${p.name.toLowerCase().replace(/[^a-z0-9]+/g,"-")}`} className="card" key={p.name}>
        <div className="imageBox"><img src={p.img} alt={p.name}/><span className="badge">{i<2?"BEST SELLER":"NEW"}</span><button aria-label="Wishlist"><Heart size={17}/></button></div>
    ...
    <section id="builder" className="builder"><span>PC COMPATIBILITY ENGINE</span><h2>Build without the guesswork.</h2><p>Choose a CPU and we’ll narrow down compatible motherboards, memory, cooling, power, and cases.</p><Link href="/builder">Start Your Build</Link></section>

    <section className="seo"><h2>Find the right hardware faster</h2><div className="seoGrid"><Link href="/guides/best-gaming-processor">Best Gaming Processors</Link><Link href="/guides/how-to-choose-gpu">How to Choose a GPU</Link><Link href="/gaming-pc/high-end">High-End Gaming Build</Link><Link href="/brand/asus">Shop ASUS Gaming</Link></div></section>
